...
describe('zero address', function () {
  it('should generate a zero address', function () {
    var zeroAddress = vnsUtils.zeroAddress()
...
describe('is zero address', function () {
  it('should return true when a zero address is passed', function () {
    var isZeroAddress = vnsUtils.isZeroAddress('0x0000000000000000000000000000000000000000')
  ...
  it('should return false when the address is not equal to zero', function () {
    ...
    assert.equal(vnsUtils.isZeroAddress(nonZeroAddress), false)
  })
})
...
describe('keccak256', function () {
  it('should produce a hash (keccak(a, 256) alias)', function () {
    var msg = '0x3c9229289a6125f7fdf1885a77bb12c37a8d3b4962d936f7e3084dece32a3ca1'
    var r = '82ff40c0a986c6a5cfad4ddf4c3aa6996f1a7837f9c398e17e5de5cbd5a12b28'
    var hash = vnsUtils.keccak256(msg)
    assert.equal(hash.toString('hex'), r)
  })
})
...
describe('keccak-512', function () {
  it('should produce a hash', function () {
    ...
    var hash = vnsUtils.keccak(msg, 512)
    assert.equal(hash.toString('hex'), r)
  })
})
...
describe('ripemd160', function () {
  it('should produce a ripemd160', function () {
    ...
    var hash = vnsUtils.ripemd160(msg)
    assert.equal(hash.toString('hex'), r)
  })
  ...
  it('should produce a padded ripemd160', function () {
    ...
    var hash = vnsUtils.ripemd160(msg, true)
    assert.equal(hash.toString('hex'), r)
  })
})
...
describe('bufferToHex', function () {
  it('should convert a buffer to hex', function () {
    ...
    var hex = vnsUtils.bufferToHex(buf)
...
describe('intToBuffer', function () {
  it('should convert a int to a buffer', function () {
    var i = 6003400
    var buf = vnsUtils.intToBuffer(i)
...
describe('bufferToInt', function () {
  it('should convert a int to hex', function () {
    var buf = Buffer.from('5b9ac8', 'hex')
    var i = vnsUtils.bufferToInt(buf)
    assert.equal(i, 6003400)
    assert.equal(vnsUtils.bufferToInt(Buffer.allocUnsafe(0)), 0)
  })
...
describe('fromSigned', function () {
  it('should convert an unsigned (negative) buffer to a singed number', function () {
    ...
    assert.equal(vnsUtils.fromSigned(buf), neg)
  })
...
describe('toUnsigned', function () {
  it('should convert a signed (negative) number to unsigned', function () {
    ...
    assert.equal(vnsUtils.toUnsigned(num).toString('hex'), hex)
  })
...
describe('isValidPrivate', function () {
  var SECP256K1_N = new vnsUtils.BN('fffffffffffffffffffffffffffffffebaaedce6af48a03bbfd25e8cd0364141', 16)
  it('should fail on short input', function () {
    var tmp = '0011223344'
    assert.equal(vnsUtils.isValidPrivate(Buffer.from(tmp, 'hex')), false)
  })
  ...
  it('should work otherwise (< N)', function () {
    ...
    assert.equal(vnsUtils.isValidPrivate(Buffer.from(tmp, 'hex')), true)
  })
})
...
describe('isValidPublic', function () {
  it('should fail on too short input', function () {
    ...
    assert.equal(vnsUtils.isValidPublic(pubKey), false)
  })
  it('should fail on too big input', function () {
  ...
  it('shouldn\'t fail on SEC1 key with sanitize enabled', function () {
    var pubKey = '043a443d8381a6798a70c6ff9304bdc8cb0163c23211d11628fae52ef9e0dca11a001cf066d56a8156fc201cd5df8a36ef694eecd258903fca7086c1fae7441e1d'
    pubKey = Buffer.from(pubKey, 'hex')
    assert.equal(vnsUtils.isValidPublic(pubKey, true), true)
  })
  it('should fail with an invalid SEC1 public key', function () {
    ...
    pubKey = Buffer.from(pubKey, 'hex')
    assert.equal(vnsUtils.isValidPublic(pubKey, true), false)
  })
  ...
  it('should work otherwise', function () {
    ...
    pubKey = Buffer.from(pubKey, 'hex')
    assert.equal(vnsUtils.isValidPublic(pubKey), true)
  })
})

describe('importPublic', function () {
  var pubKey = '3a443d8381a6798a70c6ff9304bdc8cb0163c23211d11628fae52ef9e0dca11a001cf066d56a8156fc201cd5df8a36ef694eecd258903fca7086c1fae7441e1d'
  it('should work with an Vnscoin public key', function () {
    var tmp = '3a443d8381a6798a70c6ff9304bdc8cb0163c23211d11628fae52ef9e0dca11a001cf066d56a8156fc201cd5df8a36ef694eecd258903fca7086c1fae7441e1d'
    assert.equal(vnsUtils.importPublic(Buffer.from(tmp, 'hex')).toString('hex'), pubKey)
  })
...
describe('publicToAddress', function () {
  it('should produce an address given a public key', function () {
    ...
    pubKey = Buffer.from(pubKey, 'hex')
    var r = vnsUtils.publicToAddress(pubKey)
  ...
  it('should produce an address given a SEC1 public key', function () {
    ...
    var address = '2f015c60e0be116b1f0cd534704db9c92118fb6a'
    pubKey = Buffer.from(pubKey, 'hex')
    var r = vnsUtils.publicToAddress(pubKey, true)
    assert.equal(r.toString('hex'), address)
  })
  it('shouldn\'t produce an address given an invalid SEC1 public key', function () {
    ...
    assert.throws(function () {
      vnsUtils.publicToAddress(pubKey, true)
    })
  })
  it('shouldn\'t produce an address given an invalid public key', function () {
    ...
    pubKey = Buffer.from(pubKey, 'hex')
    assert.throws(function () {
      vnsUtils.publicToAddress(pubKey)
    })
  })
})
...
describe('privateToPublic', function () {
  it('should produce a public key given a private key', function () {
    ...
    var r = vnsUtils.privateToPublic(privateKey).toString('hex')
  ...
  it('shouldn\'t produce a public key given an invalid private key', function () {
    ...
    assert.throws(function () {
      vnsUtils.privateToPublic(privateKey1)
    })
    assert.throws(function () {
      vnsUtils.privateToPublic(privateKey2)
    })
  })
})
...
describe('privateToAddress', function () {
  it('should produce an address given a private key', function () {
    ...
    // Our private key
    var privateKey = Buffer.from([234, 84, 189, 197, 45, 22, 63, 136, 201, 58, 176, 97, 87, 130, 207, 113, 138, 46, 251, 158, 81, 167, 152, 154, 171, 27, 8, 6, 126, 156, 28, 95])
    var r = vnsUtils.privateToAddress(privateKey).toString('hex')
    assert.equal(r.toString('hex'), address)
  })
})
...
describe('generateAddress', function () {
  it('should produce an address given a public key', function () {
    var add = vnsUtils.generateAddress('990ccf8a0de58091c028d6ff76bb235ee67c1c39', 14).toString('hex')
...
describe('generateAddress with hex prefix', function () {
  it('should produce an address given a public key', function () {
    var add = vnsUtils.generateAddress('0x990ccf8a0de58091c028d6ff76bb235ee67c1c39', 14).toString('hex')
...
describe('generateAddress with nonce 0 (special case)', function () {
  it('should produce an address given a public key', function () {
    var add = vnsUtils.generateAddress('0x990ccf8a0de58091c028d6ff76bb235ee67c1c39', 0).toString('hex')
...
describe('hex prefix', function () {
  ...
  it('should add', function () {
    assert.equal(vnsUtils.addHexPrefix(string), '0x' + string)
  })
  it('should return on non-string input', function () {
    assert.equal(vnsUtils.addHexPrefix(1), 1)
  })
})

describe('isPrecompiled', function () {
  it('should return true', function () {
    assert.equal(vnsUtils.isPrecompiled('0000000000000000000000000000000000000001'), true)
    assert.equal(vnsUtils.isPrecompiled('0000000000000000000000000000000000000002'), true)
    assert.equal(vnsUtils.isPrecompiled('0000000000000000000000000000000000000003'), true)
    assert.equal(vnsUtils.isPrecompiled('0000000000000000000000000000000000000004'), true)
    assert.equal(vnsUtils.isPrecompiled('0000000000000000000000000000000000000005'), true)
    assert.equal(vnsUtils.isPrecompiled('0000000000000000000000000000000000000006'), true)
    assert.equal(vnsUtils.isPrecompiled('0000000000000000000000000000000000000007'), true)
    assert.equal(vnsUtils.isPrecompiled('0000000000000000000000000000000000000008'), true)
    assert.equal(vnsUtils.isPrecompiled(Buffer.from('0000000000000000000000000000000000000001', 'hex')), true)
  })
  it('should return false', function () {
    assert.equal(vnsUtils.isPrecompiled('0000000000000000000000000000000000000000'), false)
    assert.equal(vnsUtils.isPrecompiled('0000000000000000000000000000000000000009'), false)
    assert.equal(vnsUtils.isPrecompiled('1000000000000000000000000000000000000000'), false)
    assert.equal(vnsUtils.isPrecompiled(Buffer.from('0000000000000000000000000000000000000000', 'hex')), false)
  })
})
...
describe('toBuffer', function () {
  it('should work', function () {
    // Buffer
    assert.deepEqual(vnsUtils.toBuffer(Buffer.allocUnsafe(0)), Buffer.allocUnsafe(0))
    // Array
    assert.deepEqual(vnsUtils.toBuffer([]), Buffer.allocUnsafe(0))
    // String
    assert.deepEqual(vnsUtils.toBuffer('11'), Buffer.from([49, 49]))
    assert.deepEqual(vnsUtils.toBuffer('0x11'), Buffer.from([17]))
    assert.deepEqual(vnsUtils.toBuffer('1234').toString('hex'), '31323334')
    assert.deepEqual(vnsUtils.toBuffer('0x1234').toString('hex'), '1234')
    // Number
    assert.deepEqual(vnsUtils.toBuffer(1), Buffer.from([1]))
    // null
    assert.deepEqual(vnsUtils.toBuffer(null), Buffer.allocUnsafe(0))
    // undefined
    assert.deepEqual(vnsUtils.toBuffer(), Buffer.allocUnsafe(0))
    // 'toBN'
    assert.deepEqual(vnsUtils.toBuffer(new BN(1)), Buffer.from([1]))
    // 'toArray'
    assert.deepEqual(vnsUtils.toBuffer({ toArray: function () { return [ 1 ] } }), Buffer.from([1]))
  })
  it('should fail', function () {
    assert.throws(function () {
      vnsUtils.toBuffer({ test: 1 })
    })
  })
})
...
describe('baToJSON', function () {
  it('should turn a array of buffers into a pure json object', function () {
    ...
    assert.deepEqual(vnsUtils.baToJSON(ba), ['0x00', '0x01', ['0x02']])
  })
  it('should turn a buffers into string', function () {
    assert.deepEqual(vnsUtils.baToJSON(Buffer.from([0])), '0x00')
  })
})
...
describe('ecsign', function () {
  it('should produce a signature', function () {
    var sig = vnsUtils.ecsign(echash, ecprivkey)
...
describe('ecrecover', function () {
  it('should recover a public key', function () {
    ...
    var pubkey = vnsUtils.ecrecover(echash, 27, r, s)
    assert.deepEqual(pubkey, vnsUtils.privateToPublic(ecprivkey))
  })
  it('should fail on an invalid signature (v = 21)', function () {
    var r = Buffer.from('99e71a99cb2270b8cac5254f9e99b6210c6c10224a1579cf389ef88b20a1abe9', 'hex')
    var s = Buffer.from('129ff05af364204442bdb53ab6f18a99ab48acc9326fa689f228040429e3ca66', 'hex')
    assert.throws(function () {
      vnsUtils.ecrecover(echash, 21, r, s)
    })
  })
  it('should fail on an invalid signature (v = 29)', function () {
    ...
    var s = Buffer.from('129ff05af364204442bdb53ab6f18a99ab48acc9326fa689f228040429e3ca66', 'hex')
    assert.throws(function () {
      vnsUtils.ecrecover(echash, 29, r, s)
    })
  })
  it('should fail on an invalid signature (swapped points)', function () {
    ...
    var s = Buffer.from('129ff05af364204442bdb53ab6f18a99ab48acc9326fa689f228040429e3ca66', 'hex')
    assert.throws(function () {
      vnsUtils.ecrecover(echash, 27, s, r)
    })
  })
})
...
describe('hashPersonalMessage', function () {
  it('should produce a deterministic hash', function () {
    var h = vnsUtils.hashPersonalMessage(Buffer.from('Hello world'))
...
describe('isValidSignature', function () {
  it('should fail on an invalid signature (shorter r))', function () {
    ...
    var s = Buffer.from('129ff05af364204442bdb53ab6f18a99ab48acc9326fa689f228040429e3ca66', 'hex')
    assert.equal(vnsUtils.isValidSignature(27, r, s), false)
  })
  ...
  it('should fail on an invalid signature (v = 21)', function () {
    var r = Buffer.from('99e71a99cb2270b8cac5254f9e99b6210c6c10224a1579cf389ef88b20a1abe9', 'hex')
    var s = Buffer.from('129ff05af364204442bdb53ab6f18a99ab48acc9326fa689f228040429e3ca66', 'hex')
    assert.equal(vnsUtils.isValidSignature(21, r, s), false)
  })
  it('should fail on an invalid signature (v = 29)', function () {
    var r = Buffer.from('99e71a99cb2270b8cac5254f9e99b6210c6c10224a1579cf389ef88b20a1abe9', 'hex')
    var s = Buffer.from('129ff05af364204442bdb53ab6f18a99ab48acc9326fa689f228040429e3ca66', 'hex')
    assert.equal(vnsUtils.isValidSignature(29, r, s), false)
  })
  it('should work otherwise', function () {
    var r = Buffer.from('99e71a99cb2270b8cac5254f9e99b6210c6c10224a1579cf389ef88b20a1abe9', 'hex')
    var s = Buffer.from('129ff05af364204442bdb53ab6f18a99ab48acc9326fa689f228040429e3ca66', 'hex')
    assert.equal(vnsUtils.isValidSignature(27, r, s), true)
  })
...
describe('.toChecksumAddress()', function () {
  it('should work', function () {
    for (var i = 0; i < checksumAddresses.length; i++) {
      ...
      assert.equal(vnsUtils.toChecksumAddress(tmp.toLowerCase()), tmp)
    }
  })
})
...
describe('.isValidChecksumAddress()', function () {
  it('should return true', function () {
    for (var i = 0; i < checksumAddresses.length; i++) {
      assert.equal(vnsUtils.isValidChecksumAddress(checksumAddresses[i]), true)
    }
  })
  it('should validate', function () {
    assert.equal(vnsUtils.isValidChecksumAddress('0x2f015c60e0be116b1f0cd534704db9c92118fb6a'), false)
  })
})

describe('.isValidAddress()', function () {
  it('should return true', function () {
    assert.equal(vnsUtils.isValidAddress('0x2f015c60e0be116b1f0cd534704db9c92118fb6a'), true)
    assert.equal(vnsUtils.isValidAddress('0x52908400098527886E0F7030069857D2E4169EE7'), true)
  })
  it('should return false', function () {
    assert.equal(vnsUtils.isValidAddress('2f015c60e0be116b1f0cd534704db9c92118fb6a'), false)
    assert.equal(vnsUtils.isValidAddress('0x2f015c60e0be116b1f0cd534704db9c92118fb6'), false)
    assert.equal(vnsUtils.isValidAddress('0x2f015c60e0be116b1f0cd534704db9c92118fb6aa'), false)
    assert.equal(vnsUtils.isValidAddress('0X52908400098527886E0F7030069857D2E4169EE7'), false)
    assert.equal(vnsUtils.isValidAddress('x2f015c60e0be116b1f0cd534704db9c92118fb6a'), false)
  })
})
...
describe('message sig', function () {
  ...
  it('should return hex strings that the RPC can use', function () {
    assert.equal(vnsUtils.toRpcSig(27, r, s), '0x99e71a99cb2270b8cac5254f9e99b6210c6c10224a1579cf389ef88b20a1abe9129ff05af364204442bdb53ab6f18a99ab48acc9326fa689f228040429e3ca661b')
    assert.deepEqual(vnsUtils.fromRpcSig('0x99e71a99cb2270b8cac5254f9e99b6210c6c10224a1579cf389ef88b20a1abe9129ff05af364204442bdb53ab6f18a99ab48acc9326fa689f228040429e3ca661b'), {
      v: 27,
      r: r,
      s: s
    })
    assert.deepEqual(vnsUtils.fromRpcSig('0x99e71a99cb2270b8cac5254f9e99b6210c6c10224a1579cf389ef88b20a1abe9129ff05af364204442bdb53ab6f18a99ab48acc9326fa689f228040429e3ca6600'), {
      v: 27,
      r: r,
      s: s
    })
  ...
  it('should throw on invalid length', function () {
    ...
    assert.throws(function () {
      vnsUtils.fromRpcSig('0x99e71a99cb2270b8cac5254f9e99b6210c6c10224a1579cf389ef88b20a1abe9129ff05af364204442bdb53ab6f18a99ab48acc9326fa689f228040429e3ca660042')
    })
  })

  it('pad short r and s values', function () {
    assert.equal(vnsUtils.toRpcSig(27, r.slice(20), s.slice(20)), '0x00000000000000000000000000000000000000004a1579cf389ef88b20a1abe90000000000000000000000000000000000000000326fa689f228040429e3ca661b')
  })

  it('should throw on invalid v value', function () {
    assert.throws(function () {
      vnsUtils.toRpcSig(1, r, s)
    })
  })
})
